...
| ------------------------------------- | ------------------------------------------------------------ |
| **Runs a consultancy**                | MULTIPLIER EFFECT - can bring MotherDuck to multiple clients |
| **"Reducing Snowflake spend by 20%"** | Already cost-focused - we can offer 70% savings              |
| **Works with 1TB+ datasets**          | Sweet spot for MotherDuck                                    |
...
I noticed you've been helping clients cut Snowflake costs.
We're seeing teams save 70% compared to Snowflake—figured
it might be relevant for your consulting work.
...
I saw you're running Full Stack Data Solutions—helping
companies with data infrastructure. A lot of consultants
we work with use MotherDuck as their go-to recommendation
for clients who don't need petabyte scale.
...
"When a client comes to you with analytics needs, what do you
typically recommend?"
```
...
"Are most of your clients actually at petabyte scale, or is
Snowflake overkill for what they need?"
```
...
"How many users are usually querying the data?"
```
...
It's simpler to set up, way cheaper than Snowflake, and
you look like a hero when you cut their costs 70%."
```
...
compute costs. You only pay when queries run.

For a client spending $2K/month on Snowflake, they might
spend $400-600 on MotherDuck for the same workload."
```
...
own isolated compute. Sub-100ms cold starts. No warehouse
babysitting.

For 99% of analytical workloads, it's faster and way simpler."
```
...
users could ask questions in plain English and get answers
from their data.
...
The question is: are they at petabyte scale? Most teams
aren't. They store a lot of data, but day-to-day queries
hit maybe 50GB-200GB.

For that, they're paying for capacity they don't need.
Would it help to show a comparison for one of your clients?"
```
...
Founded by the team behind Google BigQuery, with leaders
from Snowflake and Databricks. Backed by a16z and Redpoint.

But the real proof is trying it—our free tier lets you
test with a client project risk-free."
```
...
If a client truly needs complex governance at petabyte scale,
...
They're technical—can show you the architecture and help
you figure out which client projects would be a fit.

How's next week look?"
```
...
Client focus: Managed data services for consulting clients
Current stack: Snowflake (for main client)
Data volumes: Not specified - lakehouse/data repository use case
Pain points: Cost (interested in cheaper alternative)

Snowflake usage:
- Clients on Snowflake: Y (main client)
- Cost concerns: Y (main driver)
- Complexity concerns: Not discussed

AI interest:
- CEO Sidekick: Not discussed this call
- MCP/PROMPT QUERY interest: Not discussed yet

What she DIDN'T know: Dashboard/customer-facing capabilities

Next steps: SA Demo scheduled
Follow-up date: Check calendar
```
...
*Prep based on MotherDuck Calls Playbook patterns*
